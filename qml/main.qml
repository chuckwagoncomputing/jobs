...
ApplicationWindow {
	...
	property var customLabels: settings.customLabels.split(",").filter(function (s) {
																														return s != ""
																													})
	...
	header: ToolBar {
		...
		ToolButton {
			...
			onClicked: {
				...
				QmlBridge.loadJobs(settings.jobDbType, settings.jobDbHost,
																							settings.jobDbPort, settings.jobDbName,
																							settings.jobDbUsername, settings.jobDbPassword)
				QmlBridge.loadCustomers(settings.customerUrl, settings.customerUsername,
